...
# Ensure GUILE_LOAD_PATH includes the project root for project-specific modules.
# The system's guile will provide its own module paths.
# Ensure GUILE_LOAD_PATH includes the project's source directory for project-specific modules.
# The system's guile should provide its own module paths for core Guix modules.
export GUILE_LOAD_PATH := $(SRC_DIR):$(GUILE_LOAD_PATH)
